...
//          $ra_id = $_GET["ra_id"]; //Get query from parameter
  ...
  $app->get('/{ra_id}/get_residents', function ($request, $response, $args) {
              ...
              // $current_id = $_GET["resident_id"]; //Get query from parameter
              $ra_id=$args['ra_id'];
              $query = "SELECT student.firstname, student.lastname, student.email, student.phone, student.room, student.picture FROM ra JOIN student ON substr(ra.room, 1, 1)=substr(student.room, 1, 1) AND ra.dorm=student.dorm WHERE ra.id=:ra_id;";
              $sth = $this->db->prepare($query);
              $sth->execute(array(":ra_id"=>$ra_id));
              try
              {
...
$app->post('/{id}/update',function($request,$response,$args){

	parse_str(file_get_contents("php://input"),$_REQUEST2);
	$data=$_REQUEST2['picture'];
	$id=$args['id'];
        $query = "SELECT * FROM ra WHERE id=:id";
        $sth = $this->db->prepare($query);
        $sth->execute(array(":id" => $id));
...
//	$picture = $data;
	if($pass)	//Password is given
	{
        	$query = "UPDATE $table SET email=:email, password=:pass, phone=:phone, room=:room, ename=:ename, erelation=:erelation, ephone=:ephone, picture=:picture WHERE id=:id";
	        $sth = $this->db->prepare($query);
	        try
		{
			$sth->execute(array(":email" => $email, ":pass" => $pass, ":phone" => $phone, ":id" => $id, ":room" => $room, ":ename" => $ename, ":erelation" => $erelation, ":ephone" => $ephone, ":picture" => $picture));
	        	$todos = array(array("success" => True));
		}
		catch(PDOException $e)
	        {
	...
	{
                $query = "UPDATE $table SET email=:email, phone=:phone, room=:room, ename=:ename, erelation=:erelation, ephone=:ephone, picture=:picture WHERE id=:id;";
                $sth = $this->db->prepare($query);
                try
                {
                        $sth->execute(array(":email" => $email, ":phone" => $phone, ":id" => $id, ":room" => $room, ":ename" => $ename, ":erelation" => $erelation, ":ephone" => $ephone, ":picture" => $picture));
